...
Une seule vie. **6 voitures** (LA HONTE + LES CINQ, 2026-09-24), UNE SILHOUETTE CHACUNE, **maçonnées en
voxels** — voir « LA GAMME EN BRIQUES » plus bas (roster 24 → 15 le 2026-08-05, puis 15 → 6 : « à part la
...
- **LA GAMME EN BRIQUES (2026-09-24) — LA HONTE + LES CINQ** — ⚠ **CE BLOC REMPLACE la gamme des 15 décrite
  juste en dessous** (gardée pour l'historique : ses 14 gabarits ont été SUPPRIMÉS du code, `git log` les a).
  Demande du user : « enlève tous les coins arrondis (faisons du pixel) · à part la honte tout est fade · des
  voitures colorées inspirées de la vraie vie, associées à l'argent, silhouette iconique · fais-en 5 ».
  **LA GAMME** : 0 LA HONTE (INTOUCHÉE, demande explicite — elle garde son épave en boîtes lisses) · 1 **ELDORADO**
  (Cadillac Eldorado 59, rose, décapotable, ailerons + obus de feux, flancs blancs, calandre en damier) ·
  2 **LE CAÏD** (Mercedes G63, vert acide, passages de roue CARRÉS, clignotants sur les ailes, charnières,
  sorties latérales, roue de secours sur la porte) · 3 **SA MAJESTÉ** (Rolls Phantom, bleu roi/ivoire,
  liseré d'or, calandre temple qui dépasse, statuette, arête d'inox) · 4 **ROSSO 40** (F40, rouge, Lexan à
  persiennes, NACA, aileron plateau NÉ des ailes) · 5 **POSTER 84** (Countach, jaune, passages TRAPÉZOÏDAUX,
  ...
  **LE MAÇON (`voxGeo`, `voxMat`, `VOX_CAR`, `VX_AO`, `VXU`)** : un gabarit voxel est une fonction
  `f(x,y,z)` qui rend, pour le CENTRE de chaque brique, une clé de palette (`{g: matière, c: couleur}`) ou 0 —
  une fonction et pas une liste de boîtes parce qu'elle sait suivre une pente, et l'escalier obtenu EST le
  style. `K.vox({hx,y1,z0,z1,f,pal})` ; les roues via `A.pix={R,w,f,pal}` (disque maçonné dans le plan YZ,
  une géométrie partagée par les 4, `rotation.x` de la boucle inchangée), essieux asymétriques `wzF`/`wzR`.
  Une gabarit à `pix` coupe tout accessoire lisse (`lamps/pipes/sp/split/doors/slats` à 0). ⚠ **UNE SEULE
  BRIQUE, 7,5 cm**, pour les 5 caisses ET leurs roues ; grille ANCRÉE (y=0 au sol, x centré sur une colonne
  impaire, z sur un multiple de la brique). ⚠ ON NE POSE QUE LA COQUILLE (face émise si la voisine est vide).
  ⚠ **OCCLUSION AMBIANTE CUITE DANS LES SOMMETS** (façon MagicaVoxel, diagonale du quad retournée selon
  l'ombre) : c'est elle qui fait « jouet laqué » au lieu de « tas de cubes » — dosée BAS (`.64/.78/.90/1`) :
  à `.56` chaque marche d'une pente douce se soulignait d'un trait noir (le nez de la Countach en mille-feuille).
  ⚠ UNE GÉOMÉTRIE PAR MATIÈRE (laque `p`, chrome `c`, or `o`, satiné `s`, mat `m`, verre `g`, feux `l`) :
  la couleur vit dans les sommets, donc bicolore/liseré/capots = même draw call. Mesuré : **12 000 à
  20 000 triangles** par caisse (≈ une grappe de raisin du décor). ⚠ **TROIS RÉGLAGES DE MATIÈRE APPRIS À
  L'ÉCRAN** : (1) le **CHROME ne se fait pas au reflet** — une face voxel est un plan parfait qui renvoie
  UNE couleur de ciel, nuit violette : à 80 % de reflet pare-chocs et jantes sortaient en plomb ; 36 % +
  plancher lumineux `uGlow` .20 = argent ; (2) **spéculaire de laque BAS et LARGE** (0x3c, 48) : un point
  chaud serré allumait EN ENTIER chaque petite marche de capot quand la caisse tournait ; (3) `uGlow`
  (part de couleur rendue sans lumière, .14 sur la laque) : sans lui, de nuit, le Giallo sortait moutarde.
  ⚠ **UNE VITRE EN BRIQUES DOIT RESTER PROCHE DE LA VERTICALE** : couchée à 30°, le pare-brise de
  l'Eldorado devenait un escalier de lames sombres (on lisait des persiennes). ⚠ les motifs (calandres,
  persiennes) se calculent sur `Math.round(ax/V)` : sur la grille centrée ax = m·V à l'epsilon près, un
  `floor` tombe une fois sur deux à côté. ⚠ une ligne fine (liseré, jonc) s'écrit sur UNE brique : `[a,a+V)`.
  ⚠ `buildCar` LIBÈRE désormais géométries et matières voxel (`userData.vox`) avant chaque rebuild.
  **DOUBLONS MESURÉS** : 15 paires, **0 ≥ 80 %**, pire ROSSO 40 × POSTER 84 à 76,5 %.
  **SAUVEGARDE** : `gamme:2` dans `SAVE.d`. Une sauvegarde d'avant (index des 15) est migrée dans `san()` :
  `owned`/`floor`/`equipped`/`bestCar` remis à zéro ET les achats REMBOURSÉS à la banque au prix d'alors
  (on ne confisque pas). `signatureBoom` (butin d'explosion par caisse) remappé sur les 6 index.
  **L'ÉTAL (`voitures.html`)** a gagné : 7 vues d'un clic (3/4 AV, FACE, PROFIL, ARRIÈRE, 3/4 AR, DESSUS,
  DESSOUS) qui glissent, déplacement (clic droit / maj+glisser), double-clic = viser un détail et s'en
  approcher, zoom jusqu'à ×0,12, touche R = recentrer, fond clair (◐), planche en 3 colonnes, prix affiché.
