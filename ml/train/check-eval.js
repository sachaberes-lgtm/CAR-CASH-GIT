...
/* ================================================================================================
   ...
   Ce test le verifie MECANIQUEMENT, de facon deterministe (pas de "regarde a l ecran") :
     ...
     2. a la selection, la fitness de chaque bot est EXACTEMENT la moyenne arithmetique
        (fitnessSum / evalCount) de ses manches, et evalCount == EVAL_TRACKS ;
...
/* ---- instrumentation : on note la graine de piste a chaque manche, et la moyenne a la selection ---- */
...
T.newGen=function(first){
  if(!first){
    // la selection arrive : chaque bot doit porter fitness = moyenne exacte de ses manches
    for(const b of T.bots){
      if(b.evalCount!==T.EVAL_TRACKS) erreurs.push('bot evalCount '+b.evalCount+' != '+T.EVAL_TRACKS);
      const attendue=b.fitnessSum/Math.max(1,b.evalCount);
      if(Math.abs(b.fitness-attendue)>1e-9) erreurs.push('bot fitness '+b.fitness+' != moyenne '+attendue);
    }
  }
...
console.log('\nPASS — chaque generation court '+TRACKS+' pistes distinctes, la note de selection est la moyenne');
console.log('       exacte de ces manches, et aucune piste n est vue deux fois d une generation a l autre.');
process.exit(0);
